/**
 * Created by betrayer on 22.02.15.
 */
"use strict";
var fs = require('fs');
var encoding = require('encoding');

var Dictionary = function(options, callback) {
    var that = this;
...
Dictionary.prototype.getOptions = function(options) {
...
Dictionary.prototype.parse = function(string, deep) {
    var that = this;
    var regexp;
    var answer = [];
...
Dictionary.prototype.queryLemma = function(lemma, ending, key) {
    var that = this;
    var answer = [];
    key = key || "";
...
Dictionary.prototype.removePrefix = function(string) {
...
Dictionary.prototype.check = function(options) {
    if (!options.dic) {
        return 1;
    }
    if (!options.gram) {
    ...
    if (!options.lang || !Dictionary.langs[options.lang]) {
        options.lang = "en";
...
Dictionary.prototype.exit = function (err) {
...
Dictionary.prototype.import = function(options, callback) {
    var that = this;
    var source = {};
    if (!options || typeof options == "function") {
        callback = options;
    ...
    fs.readFile(options.dic, function(err, data) {
        ...
        if (options.lang !== "en") {
            data = encoding.convert(data, "utf8", Dictionary.langs[options.lang].encoding);
        }
                ...
                if (reg.test(split[i].slice(0, 2))) {
                    ...
                    if (rule[3]) {
                        ...
                        for (var j = 0; j < temp.length; ++j) {
                            var rul = Dictionary.gmap[options.lang][temp[j]];
                            if (rul) {
                                info.push(rul);
                            }
                        }
                    ...
                    source.rules[rule[0]] = {
                        type: Dictionary.gmap[options.lang][rule[2]] || 'All',
                        info: info
                    };
                    source.counter++;
                }
...
Dictionary.prototype.mine = function(options, callback) {
    var that = this;
...
Dictionary.prototype.store = function(path, callback) {
    var that = this;
